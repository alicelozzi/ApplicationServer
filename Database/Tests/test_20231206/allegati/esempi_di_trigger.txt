...
BEGIN
	DECLARE numero_sostenimenti INT DEFAULT 0;
	-- numero sostenimenti studente (numero di esami sotenuti con voto registrato per lo studente con matricola)

	-- il cursore rappresenta una variabile tabella in memoria RAM
	DECLARE cursore_sostenimenti_studente CURSOR FOR   
		select COUNT(*) as numero_sostenimenti_studente
		  from sostenimento 
		 where so.matricola = new.matricola  -- valore matricola del nuovo record che si sta inserendo
				

 	open cursore_sostenimenti_studente 
 	fetch cursore_sostenimenti_studente into numero_sostenimenti; 
 	close cursore_sostenimenti_studente 
	
	IF (numero_sostenimenti >= 2) THEN
 		SIGNAL SQLSTATE '75000' SET MESSAGE_TEXT="Lo studente ha già due sostenimenti presenti!!!"; 
 	END IF;
...
CREATE DEFINER=`root`@`localhost` TRIGGER `ImpedimentoTv`
BEFORE INSERT ON ordinazione FOR EACH ROW
BEGIN
	IF (SELECT SUBSTRING(new.codice_prodotto,1,2) = 'TV') THEN
		SIGNAL SQLSTATE '75000' SET MESSAGE_TEXT="ordinazione non registrabile con codice TV";
	END IF;
END
