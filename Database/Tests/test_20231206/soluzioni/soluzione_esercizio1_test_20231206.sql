...
BEGIN
	...
	-- il cursore rappresenta una variabile tabella in memoria RAM
	DECLARE cursore_calcolo_km_percorsi_mezzo_con_limite CURSOR FOR   
		SELECT mezzo.limite_km_percorribili 
			 , ( (SUM(spostamento.km_percorsi) + new.km_percorsi) / mezzo.capienza_massima) AS km_percorsi_dal_mezzo
		  FROM mezzo left join spostamento on spostamento.codice_mezzo = mezzo.codice_mezzo  
		 WHERE spostamento.codice_mezzo = new.codice_mezzo;
		 
				
 	OPEN cursore_calcolo_km_percorsi_mezzo_con_limite; 
 	FETCH cursore_calcolo_km_percorsi_mezzo_con_limite into limite_km_percorribili_mezzo, km_percorsi_mezzo; 
 	CLOSE cursore_calcolo_km_percorsi_mezzo_con_limite; 
	
	IF (km_percorsi_mezzo > limite_km_percorribili_mezzo) THEN
		SIGNAL SQLSTATE '75000' SET MESSAGE_TEXT="impossibile inserire lo spostamento per il mezzo: il mezzo ha superato il suo limite di km percorribili!!!";
	END IF;

END 
	 
-- ai fini del test: occorre ridurre ad un valore molto basso o porre a zero 
-- il limite percorribili per un mezzo determinato, così da  verificare che 
-- quando si supera il limte il trigger scatta impedendo...	 

UPDATE mezzo 
   SET limite_km_percorribili = 0
 WHERE mezzo.codice_mezzo = 'TM-001'

-- inserimento spostamento da impedire: mezzo con km percorsi oltre limite percorribili
INSERT INTO trasporti.spostamento 
     (data_spostamento,codice_fiscale,codice_mezzo,km_percorsi) VALUES
	 ('2023-08-18','VLDBCC0123456789','TM-001',14)
	 
-- inserimento spostamento ammesso: mezzo con km percorsi al di sotto del limite percorribili	 
INSERT INTO trasporti.spostamento 
     (data_spostamento,codice_fiscale,codice_mezzo,km_percorsi) VALUES
	 ('2023-08-18','VLDBCC0123456789','TM-002',14)
