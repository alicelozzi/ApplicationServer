...
import it.step.tutoring.applicationserver.calcolatrice.control.UtenteAnonimoServlet;
...
public class HttpServerSocket {
    ...
    private static void handleHttpRequest(Socket client) throws IOException {
        ...
        if (htmlContentPage.length == 0) {
            ...
            sendHttpResponse(client, "HTTP/1.1 404 Not Found", "text/html", notFoundContent);
        }
        else {
    ...
    /*
        INVIA LA RIPSOSTA AL CLIENT (HTTP RESPONSE)
     */
    private static void sendHttpResponse(Socket client, String status, String contentType, byte[] content) throws IOException {
        try {
            OutputStream clientOutput = client.getOutputStream();
            clientOutput.write(new String("HTTP/1.1 \r\n" + status).getBytes());              //{PROTOCOL VERSION} {STATUS} {REASON}
            clientOutput.write(new String("ContentType: " + contentType + "\r\n").getBytes());//{Content-type} header, tipo di conteuto della risposta 
            clientOutput.write("\r\n".getBytes());
